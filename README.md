...
Introduction
넷플릭스, 웨이브 등 OTT 서비스의 급성장과 팬데믹을 거치며 한국 극장 산업은 심각한 구조적 변화를 맞이했습니다. 본 프로젝트는 코로나 거품이 완전히 꺼진 엔데믹 뉴노멀(2024~2026) 시대를 기점으로, 대중의 콘텐츠 소비 중심이 어떻게 이동했는지 데이터 기반으로 증명합니다. 
...
1. Data Ingestion (수집 및 이관)
   Batch: Python으로 KOBIS API(최신 박스오피스)를 호출해 로컬 MySQL에 1차 적재 후, Apache Sqoop을 통해 HDFS(Raw Zone)로 일괄 분산 이관합니다. Netflix 데이터는 HDFS CLI로 직접 적재합니다.
...
3. Speed Layer (실시간 스트리밍 분석)
   Kafka로 유입되는 실시간 버즈 데이터를 Spark Structured Streaming이 마이크로 배치 단위로 파싱하여 인메모리 상에서 실시간 화제성 지수를 갱신합니다.
